<form class="" action='{{route("siniestros.terceros.paso6.daniomaterial.store")}}' method="post">
    @csrf
        ...
        <div class="row">
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        <select name="tipo" id="tipo"
                                class="custom-select {{ old('check_otro_tipo') ? 'd-none' :  '' }} @error('tipo') is-invalid @enderror">
                            @foreach($tipos as $tipo)
                                <option value="{{ $tipo }}"
                                    {{ old('tipo') && old('tipo') == $tipo ? 'selected' : '' }}
                                >{{ $tipo }}</option>
                            @endforeach
                        </select>
                        <input type="text" name="otro_tipo" id="otro_tipo" maxlength="255"
                               class="form-control {{ old('check_otro_tipo') ? '' : 'd-none' }} @error('otro_tipo') is-invalid @enderror"
                               value="{{ old('otro_tipo') ? old('otro_tipo') : '' }}"
                        >
                        <div class="input-group-append">
                            <div class="input-group-text">
                                <input type="checkbox" id="check_otro_tipo" name="check_otro_tipo"
                                       class="mr-1" {{ old('check_otro_tipo') ? 'checked' : '' }}>Otro
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-12">
                <div class="form-group">
                    ...
                    >{{ old('detalles') ? old('detalles') : ''  }}</textarea>
        ...
        <div class="row">
            <div class="col-12">
                <a class="mt-3 boton-enviar-siniestro btn"
                   ...
                   href='{{route('siniestros.terceros.paso6.create',['id' => request('id')])}}'>VOLVER</a>
