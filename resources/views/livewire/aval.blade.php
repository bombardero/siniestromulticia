<div class="">
    ...
    @endif	
    	<div class = "bg-form">
		    ...
		    <p class="avales-formato pt-3">Los avales son verificaciones de entrada de dinero por mes (recibos de sueldo si estás contratado o de facturación mensual si sos monotributista). Son necesarios para que Finisterre pueda emitir el seguro de caución. 
		    </p>
		   
		   <p class="text-center cargar-avales">Cargar avales</p>
		    
		        <div class="form-group row ">
		        
		        	<div class="text-center col-12 ">
					<input type="file"  id="avales" wire:change="$emit('single_file_choosed_aval')"  >
					<label for="avales">
						<div class="row">
						...
						<p>@error('avales') <span class="text-danger">{{ $message }}</span> @enderror</p>
						@foreach($solicitud->documentos()->where('type', 1)->get() as $archivo)
							<div class="row">
								<div class="col-12">
								<p>
									<a target="_blank" class="documento-formato-texto pt-2" href={{$archivo->url}}>{{$archivo->nombre}}</a><i class="pl-2 fas fa-check"></i>

									@if($solicitud->documentos()->where('type', 1)->count() > 1)
									 <button 
