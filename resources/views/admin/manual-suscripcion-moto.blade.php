        ...
        <div class="container">
            <div class="row">
                <div class="col-12 pt-5">
                    ...
                    <p class="pt-4 panel-admin-table-title">Historial de carga: Manual suscripción motos</p>
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            @if($documentos->isEmpty())
                                <td class="borde-tabla">No hay un manual de suscripción de motos cargados aún.</td>
                            @else
                                @foreach($documentos as $documento)
                                    @if($loop->first && $documentos->onFirstPage())
                                        <tr class="borde-tabla ultimo-documento">
                                            ...
                                            <td>{{$documento->id}}</td>
                                            <td><a class="ultimo-documento" target="_blank"
                                                   href="{{$documento->url}}">{{$documento->url}}<img
                                                        src="{{url('/images/admin/verificado.svg')}}"
                                                        class="pl-md-3 img-fluid "></a></td>
                                        </tr>
                                    @else
                                        <tr class="borde-tabla ">
                                            ...
                                            <td><a style="color:black;" target="_blank"
                                                   href="{{$documento->url}}">{{$documento->url}}</a></td>
                                        </tr>
                                    @endif
                                @endforeach
                            </tbody>
