...
use App\Jobs\VerificarVigenciaPolizaDenunciaSisUnico;
...
class FormAsegurados extends Component
{
    ...
    {
        $this->tipos_siniestros = DenunciaSiniestroSisUnico::TIPOS_SINIESTROS;
    }
    ...
    private function validateAsegurado()
    {
        return $validateAsegurable = $this->validate([
            ...
            'tipo_siniestro' => ['required',Rule::in(DenunciaSiniestroSisUnico::TIPOS_SINIESTROS)]
        ],
        [
    ...
    public function submit() {
        ...
        $denuncia = DenunciaSiniestroSisUnico::create([
        ...
        VerificarVigenciaPolizaDenunciaSisUnico::dispatch($denuncia);

        //compania
        Mail::to(config('app.mail_siniestro_asegurado'))->send(new MailAseguradoCompania($data));

        return redirect()->to('/graciassisunico');
    }
    ...
    {
        return view('livewire.siniestrosisunico.form-asegurados');
    }
